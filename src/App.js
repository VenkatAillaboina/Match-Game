...
const defaultValues = {
  activeTabItemId: tabsList[0].tabId,
...
class App extends Component {
  ...
  changeActiveTabItemId = tabId => {
    this.setState({activeTabItemId: tabId})
  }

  generateRandomImage = () => {
    const index = Math.floor(Math.random() * imagesList.length)
    const currentImageId = imagesList[index].id
    this.setState(prevState => ({currentImageId, score: prevState.score + 1}))
  }
  ...
  render() {
    const {activeTabItemId, currentImageId, score, timer, gameOver} = this.state
    const {imageUrl} = imagesList.find(
      eachImage => eachImage.id === currentImageId,
    )
    const filteredImagesList = imagesList.filter(
      eachImageItem => eachImageItem.category === activeTabItemId,
    )
    return (
      <>
        ...
        <div className="match-game-app-bg-container">
          {gameOver ? (
            <GameOver score={score} playAgainGame={this.playAgainGame} />
          ) : (
            <>
              <img className="match-image" src={imageUrl} alt="match" />
              <ul className="tab-and-image-items-container">
                {tabsList.map(eachTabItem => (
                  <TabItem
                    key={eachTabItem.tabId}
                    isTabItemActive={activeTabItemId === eachTabItem.tabId}
                    tabItemDetails={eachTabItem}
                    changeActiveTabItemId={this.changeActiveTabItemId}
                  />
                ))}
              </ul>
              <ul className="tab-and-image-items-container">
                {filteredImagesList.map(eachImageItem => (
                  <ImageItem
                    ...
                    changeCurrentImageID={this.changeCurrentImageID}
                  />
                ))}
              </ul>
            </>
